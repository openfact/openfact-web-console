...
import { OpenfactSearchService } from "./openfact.search.service";
...
export abstract class OrganizationResourceSearchService<T extends OpenfactResource, L extends Array<T>, S extends SearchResults<T>> extends OpenfactSearchService<T, L, S> {
  ...
  search(criteria: any = null, organization: string = null): Observable<S> {
    ...
    if (url) {
      ...
      return Observable.create((observer) => observer.next(<SearchResults<T>>{ items: [], totalSize: 0 }));
    }
  }
